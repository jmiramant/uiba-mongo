...
describe('Users Action Creators', () => {
  describe('User Login', () => {
    it('beginLogin returns action type MANUAL_LOGIN_USER', () => {
      expect(actions.beginLogin()).toEqual({type: UserTypes.MANUAL_LOGIN_USER});
    });

    it('loginSuccess returns action type LOGIN_SUCCESS_USER and a success message', () => {
      const message = 'Success';
      expect(actions.loginSuccess(message)).toEqual({type: UserTypes.LOGIN_SUCCESS_USER, message});
    });

    it('loginError returns action type LOGIN_ERROR_USER and an error message', () => {
      const message = 'Oops! Something went wrong!';
      expect(actions.loginError(message)).toEqual({type: UserTypes.LOGIN_ERROR_USER, message});
    });
  });
  describe('User Signup', () => {
    it('beginSignUp returns action type SIGNUP_USER', () => {
      expect(actions.beginSignUp()).toEqual({type: UserTypes.SIGNUP_USER});
    });
    it('signUpSuccess returns action type SIGNUP_SUCCESS_USER and a success message', () => {
      const message = 'Success';
      expect(actions.signUpSuccess(message)).toEqual({type: UserTypes.SIGNUP_SUCCESS_USER, message});
    });
    it('signUpError returns action type SIGNUP_ERROR_USER and an error message', () => {
      const message = 'Oops! Something went wrong!';
      expect(actions.signUpError(message)).toEqual({type: UserTypes.SIGNUP_ERROR_USER, message});
    });
  });
  describe('User Logout', () => {
    it('beginLogout returns action type LOGOUT_USER', () => {
      expect(actions.beginLogout()).toEqual({type: UserTypes.LOGOUT_USER});
    });
    it('signUpSuccess returns action type SIGNUP_SUCCESS_USER', () => {
      expect(actions.logoutSuccess()).toEqual({type: UserTypes.LOGOUT_SUCCESS_USER});
    });
    it('signUpError returns action type SIGNUP_ERROR_USER', () => {
      expect(actions.logoutError()).toEqual({type: UserTypes.LOGOUT_ERROR_USER});
    });
  });
});
